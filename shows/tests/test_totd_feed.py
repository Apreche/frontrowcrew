...
class TotDFeedTests(utils.FRCTestCase):
    ...
    def test_totd_feed_item_contents(self):
        ...
        item = items[0]
        self.assertEqual(item.find("title").text, related_link.title)
        self.assertEqual(item.find("description").text, related_link.description)
        self.assertEqual(item.find("link").text, related_link.url)
        self.assertEqual(
            item.find(
                "dc:creator",
