...
class ShowPodcastFeedTests(utils.FRCTestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.podcast = cls.show.podcast
        cls.live_podcast_content = []
    ...
    def test_feed_404(self):
        """
        Only shows that are podcasts should have podcast feeds
        """
    ...
    def test_itunes_link(self):
        ...
        self.assertTrue(link.text.endswith(self.show.get_absolute_url()))
    ...
    def test_item_titles(self):
        channel = self.etree.find("channel")
        self.assertIsNotNone(channel)
        for content in self.live_podcast_content:
    ...
    def test_child_show_items(self):
        ...
        try:
            ...
            exception = etree_exception
        self.assertIsNone(exception)
        channel = parent_etree.find("channel")
        self.assertIsNotNone(channel)
        for content in self.live_podcast_content:
            episode = content.podcast_episode
            item = channel.find(f"item[guid='{episode.guid}']")
